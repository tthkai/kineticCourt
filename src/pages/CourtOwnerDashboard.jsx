...
const DAILY_STATS = { revenue: '2,450,000', totalBookings: 12, occupancyRate: '75%' };
const SCHEDULE_DATA = [
...
export default function CourtOwnerDashboard() {
  ...
  return (
    // THAY ĐỔI 1: ÉP KHUNG MOBILE (maxWidth 480px và margin auto)
    <div style={{ 
      maxWidth: '480px', 
      margin: '0 auto', 
      ...
      paddingBottom: '80px',
      position: 'relative',
      boxShadow: '0 0 20px rgba(0,0,0,0.5)' // Thêm bóng đổ 2 bên cho giống app
    }}>
      ...
      <header style={{ padding: '20px', borderBottom: '1px solid #333', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#1a1a1a', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <button onClick={logout} className="icon-btn-transparent" style={{ color: '#ff4444', background: 'none', border: 'none', cursor: 'pointer' }}>
          <LogOut size={20} />
        </button>
      </header>
        ...
        {activeTab === 'schedule' && (
          <div className="fade-in">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
              <h3 style={{ margin: 0 }}>Lịch đặt sân</h3>
              <button style={{ background: '#ccff00', color: 'black', border: 'none', padding: '6px 12px', borderRadius: '6px', fontSize: '0.8rem', fontWeight: 'bold', cursor: 'pointer' }}>+ Mới</button>
            </div>
              ...
              {SCHEDULE_DATA.map(item => (
                <div key={item.id} style={{ backgroundColor: '#1e1e1e', padding: '15px', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '0.9rem', color: '#ccc' }}>Khách: {item.user}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontWeight: 'bold' }}>{item.price}</div>
                    {item.status === 'PAID' 
                      ? <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.75rem', color: '#ccff00', marginTop: '4px' }}><CheckCircle2 size={12}/> Đã thu</span>
                      : <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.75rem', color: '#ffaa00', marginTop: '4px' }}><Clock size={12}/> Chờ thu</span>
                    }
                  </div>
                </div>
              ))}
              ...
              {COURTS_DATA.map(court => (
                <div key={court.id} style={{ backgroundColor: '#1e1e1e', padding: '15px', borderRadius: '12px', borderLeft: court.status === 'ACTIVE' ? '4px solid #ccff00' : '4px solid #ff4444' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: '0.7rem', padding: '4px 8px', borderRadius: '4px', backgroundColor: court.status === 'ACTIVE' ? 'rgba(204, 255, 0, 0.1)' : 'rgba(255, 68, 68, 0.1)', color: court.status === 'ACTIVE' ? '#ccff00' : '#ff4444' }}>
                      {court.status === 'ACTIVE' ? 'SẴN SÀNG' : 'BẢO TRÌ'}
                    </span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '15px', alignItems: 'center' }}>
                    <span style={{ color: '#888', fontSize: '0.9rem' }}>Giá: {court.basePrice}/h</span>
                    <button style={{ backgroundColor: 'transparent', border: '1px solid #444', color: 'white', padding: '6px 12px', borderRadius: '6px', fontSize: '0.8rem', cursor: 'pointer' }}>Chỉnh sửa</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* THAY ĐỔI 2: NEO THANH ĐIỀU HƯỚNG THEO KHUNG MOBILE */}
      <nav style={{ 
        position: 'fixed', 
        bottom: 0, 
        left: '50%', // Đẩy ra giữa màn hình
        transform: 'translateX(-50%)', // Kéo ngược lại 1/2 kích thước để căn giữa tuyệt đối
        width: '100%', 
        maxWidth: '480px', // Bằng đúng kích thước container chính
        ...
        <button onClick={() => setActiveTab('dashboard')} style={{ background: 'transparent', border: 'none', color: activeTab === 'dashboard' ? '#ccff00' : '#888', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', cursor: 'pointer' }}>
          <LayoutDashboard size={24} />
          <span style={{ fontSize: '0.7rem' }}>Tổng quan</span>
        </button>
        <button onClick={() => setActiveTab('schedule')} style={{ background: 'transparent', border: 'none', color: activeTab === 'schedule' ? '#ccff00' : '#888', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', cursor: 'pointer' }}>
          <CalendarClock size={24} />
          <span style={{ fontSize: '0.7rem' }}>Lịch đặt</span>
        </button>
        <button onClick={() => setActiveTab('courts')} style={{ background: 'transparent', border: 'none', color: activeTab === 'courts' ? '#ccff00' : '#888', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', cursor: 'pointer' }}>
          <MapPin size={24} />
